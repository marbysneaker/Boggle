...
2.  Suppose you use a binary search tree to store the words from the 
    ...
    > it would take n-1 n being the total amount of words in the dictionary;
...
3.  Suppose you use a prefix tree to store the words from the `dictionary.txt` 
    ...
    > it would take the amount of letters in a word;

4.  Why is the prefix tree better than a binary search tree for implementing 
    Boggle?

    > because its more efficient in finding words inside a dictionary, by taking less
    time because it takes less comparison. instead of comparing to every single word we just have to 
>   traverse trough every single letter and check if isWord is already true;

5.  Suppose you do not use `IsPrefix` in your `SolveBoard` implementation.  
    How would that affect the program?

    > we will keep going even though there is no word that begins with the given letters,
    our program will take longer and can run into an infinite recursion
